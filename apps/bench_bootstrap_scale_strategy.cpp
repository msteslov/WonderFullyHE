...
void run_profile(const ProfileCase& profile_case) {
    ...
    for (double period_log2 = profile_case.min_period_log2;
        ...
        for (double max_plain_scale_log2 : {40.0, 50.0, 60.0, 80.0, 100.0, 120.0, 160.0, 200.0, 240.0}) {
            ...
            std::printf("%s,%.0f,%.0f,%zu,%zu,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%s,%zu,%zu,%zu,%zu,%zu,%.6e,%.6e,%.6e,%zu,%zu,%.6e,%.6e,%.6e,%.6e,%.6e,%s,%s,%s,%s,%s,%s\n",
                        profile_case.name,
                        period_log2,
                        max_plain_scale_log2,
                        ...
                        plan.missing_drop_log2,
                        plan.missing_scalar_levels,
                        plan.missing_total_levels,
...
int main() {
    std::printf("profile,period_log2,max_plain_scale_log2,start_chain_index,start_coeff_modulus_size,start_scale_log2,start_coeff_modulus_log2,max_abs_before_normalization,expected_max_abs_after_normalization,min_period_for_magnitude_log2,max_period_for_evalmod_capacity_log2,period_window_margin_log2,period_window_possible,max_consumable_levels,scalar_levels_needed,total_levels_needed,scale_squash_levels_needed,chain_remaining_after_strategy,required_drop_log2,available_drop_log2,missing_drop_log2,missing_scalar_levels,missing_total_levels,scale_after_scalar_log2,scale_after_squash_log2,target_scale_log2,remaining_coeff_modulus_log2_after_strategy,first_evalmod_product_scale_log2,evalmod_magnitude_ready,scale_strategy_feasible,evalmod_first_multiply_ready,p3_ready,scale_design_status,blocker\n");
