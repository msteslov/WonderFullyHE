...
Cipher SealAdapter::mod_switch_to_next_preserve_scale(const Cipher& cipher) {
    ...
    Cipher out = cipher;
    pimpl_->evaluator->mod_switch_to_next_inplace(out.pimpl_->ct);
    out.pimpl_->ct.scale() = old_scale;
    return out;
}
...
Cipher SealAdapter::mul_by_uint64_no_rescale(const Cipher& cipher, std::uint64_t scalar) {
    ...
    const std::size_t modulus_size = moduli.size();
    for (std::size_t poly_index = 0; poly_index < out.pimpl_->ct.size(); ++poly_index) {
        auto* poly = out.pimpl_->ct.data(poly_index);
        for (std::size_t modulus_index = 0; modulus_index < modulus_size; ++modulus_index) {
            const auto& modulus = moduli[modulus_index];
            const std::uint64_t scalar_mod = seal::util::barrett_reduce_64(scalar, modulus);
            auto* coeffs = poly + modulus_index * coeff_count;
            for (std::size_t coeff_index = 0; coeff_index < coeff_count; ++coeff_index) {
                coeffs[coeff_index] = seal::util::multiply_uint_mod(coeffs[coeff_index], scalar_mod, modulus);
            }
        }
    }
    out.pimpl_->ct.scale() = cipher.pimpl_->ct.scale() * static_cast<double>(scalar);
    return out;
}
...
Cipher SealAdapter::bootstrap_modup_to_first(const Cipher& cipher, BootstrapModUpVariant variant) {
    ...
    if (!cipher.pimpl_->ct.is_ntt_form()) {
        throw std::invalid_argument("CKKS ciphertext must be in NTT form for bootstrap ModUp");
    }
    ...
    std::vector<std::uint64_t> half_source_modulus(source_modulus_size);
    std::vector<std::uint64_t> source_modulus_mod_extension(extension_moduli.size());
    if (variant == BootstrapModUpVariant::CenteredLift) {
        seal::util::right_shift_uint(source_base.base_prod(), 1, source_modulus_size, half_source_modulus.data());
        for (std::size_t i = 0; i < extension_moduli.size(); ++i) {
            source_modulus_mod_extension[i] =
                seal::util::modulo_uint(source_base.base_prod(), source_modulus_size, extension_moduli[i]);
