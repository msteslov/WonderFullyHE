...
m2424::BootstrapStcScaleDownResult apply_stc_scale_down_modup_reference_aligned(
    m2424::SealAdapter& adapter,
    const m2424::Cipher& after_slots_to_coeff,
    const m2424::BootstrapStcReferencePlan& reference_plan) {
    const auto info = adapter.info(after_slots_to_coeff);
    auto modup_plan = m2424::plan_stc_first_modup(info, reference_plan.message_scale_log2, reference_plan.message_scale_log2);
    modup_plan.scale_down.scale_down_log2 = reference_plan.scale_down_log2;
    modup_plan.scale_down.expected_period_log2 = reference_plan.modup_period_log2;
    return m2424::apply_stc_scale_down(adapter, after_slots_to_coeff, modup_plan.scale_down);
}
...
void run_case(const char* input_mode, bool complex_mode) {
    ...
    const auto scaled = apply_stc_scale_down_modup_reference_aligned(adapter, current, reference_plan);
    ...
    const auto encrypted_z = head(adapter.decode_complex(adapter.decrypt(current)));

    const auto encrypted_scan = m2424::scan_bootstrap_lattice_periods(encrypted_z, input, 80.0, 115.0, kUsefulGain);
    const auto reference_scan = m2424::scan_bootstrap_lattice_periods(
        reference.after_coeff_to_slots, input, 80.0, 115.0, kUsefulGain);
    ...
    std::printf("[diagnose_bootstrap_encrypted_vs_reference] input_mode=%s reference_pass=%s reference_best_p=%.0f reference_gamma_abs=%.12e reference_gamma_arg=%.12e reference_err_mod_gain=%.12e encrypted_has_useful=%s encrypted_best_p=%.0f encrypted_gamma_abs=%.12e encrypted_gamma_arg=%.12e encrypted_err_mod_direct=%.12e encrypted_err_mod_gain=%.12e encrypted_vs_reference_abs_error=%.12e encrypted_vs_reference_fractional_error=%.12e after_stc_chain=%zu after_stc_scale_log2=%.6f after_scale_down_chain=%zu after_scale_down_scale_log2=%.6f after_cts_chain=%zu after_cts_scale_log2=%.6f classification=\"%s\"\n",
                input_mode,
