...
m2424::CkksProfile bsgsCalibrationProfile() {
    return {16384, {60, 59, 59, 60}, std::exp2(59.0), 8192};
}
...
int main() {
    ...
    for (const std::size_t size : {std::size_t{4}, std::size_t{8}, std::size_t{16}}) {
        const m2424::CoeffToSlotBsgsPlan plan(size, 8192, 4);
    ...
    auto adapter = m2424::SealAdapter::create(bsgsCalibrationProfile());
    const m2424::CoeffToSlotBsgsPlan encryptedPlan(16, adapter.slotCount(), 4);
