...
Для нетривиальных программ планировщик должен получать не только
глубину, но и бюджет операций:
\[
    B_{\mathsf{ops}} =
    (n_{\mathsf{add}}, n_{\mathsf{ptadd}}, n_{\mathsf{ctmul}},
     n_{\mathsf{ptmul}}, n_{\mathsf{rescale}}, n_{\mathsf{modswitch}},
     n_{\mathsf{rot}}, n_{\mathsf{linear}}, n_{\mathsf{evalmod}},
     n_{\mathsf{refresh}}).
\]
Именно этот бюджет определяет как потери точности, так и число рабочих
уровней chain. Грубый \(\mathsf{ops}\)-профиль используется только как
fallback для простых сценариев.

Результат планирования также содержит оценку:
\[
    E_{\mathsf{planned}} = 2^{-b_{\mathsf{work}} + b_{\mathsf{loss}}},
\]
и флаг прохождения:
\[
    E_{\mathsf{planned}} \leq \varepsilon.
\]
...
\hline
\texttt{bench\_parameter\_planner} & Проверка выбранного планировщиком профиля на SEAL-прогоне. \\
\hline
