...
Число \(S\) задаёт физическое число CKKS-слотов. Число \(m\) задаёт
логические слоты полезной нагрузки. Если используется ABFT-контрольная
сумма, требуется
\[
    m+1\leq S.
\]
...
\]
В прикладных проверках контрольная сумма считается только по первым \(m\)
логическим слотам; остальные физические слоты не входят в ABFT-инвариант.
...
Инварианты для сложения, вычитания и ротации:
\[
...
Для покомпонентного умножения контрольная сумма не выводится из одних
\(\sigma(x)\) и \(\sigma(y)\). Если перемножить расширенные векторы
\[
    (x_0,\ldots,x_{m-1},\sigma(x))
    \quad\text{и}\quad
    (y_0,\ldots,y_{m-1},\sigma(y)),
\]
то последний слот даст
\[
    \sigma(x)\sigma(y),
\]
а требуемая сумма произведения равна
\[
    \sigma(x\odot y)=\sum_{i=0}^{m-1}x_iy_i.
\]
Поэтому для умножения используется отдельная проверка: сначала вычисляется
шифртекст произведения \(c_{xy}=\Enc(x\odot y)\), затем по первым \(m\)
логическим слотам считается сумма
\[
    \Sigma_{xy}=\sum_{i=0}^{m-1}x_iy_i.
\]
Условие проверки:
\[
    |\Sigma_{\operatorname{obs}}-\Sigma_{\operatorname{exp}}|
    ...
    \tau_{\operatorname{abft}}.
\]
Здесь \(\Sigma_{\operatorname{obs}}\) получается суммированием слотов
результата умножения, а \(\Sigma_{\operatorname{exp}}\) задаёт эталонное
значение для проверяемого сценария.
...
\]
Здесь \(\Lambda\) обозначает подъём уровня шифртекста.
Масштаб сохраняется:
\[
    \scl(\Lift(c))=\scl(c),
\]
...
\[
    \lvl(\Lift(c))=L.
\]
Подъём уровня расширяет базу представления шифртекста, но сам по себе не
устраняет накопленную численную ошибку. В контуре обновления он используется
только как один из этапов вместе с переходом в слотовое представление,
полиномиальной редукцией и обратным преобразованием.
...
Такое действие является переинтерпретацией масштаба шифртекста. Оно требует
контроля совместимости масштабов на следующих операциях и может влиять на
ошибку декодирования, поэтому в отчёте фиксируются масштаб до этапа, масштаб
после этапа и итоговая ошибка.

\section*{13. Прототип контура обновления шифртекста}

Пусть входной шифртекст \(c_0\) находится на уровне \(\ell<L\).
Экспериментальный контур обновления задаётся последовательностью:
\[
    c_1=\Lift(c_0),
\]
\[
...
Математически библиотечное вычисление задаётся отображением
\[
    \operatorname{Alg}:
    (x,P,\tau)
    \mapsto
...
\]
где \(f\) --- целевая функция, которую требуется вычислить над исходными
данными.
